/*
 * File:   LCD_Displaycode.c
 * Author: 2231349
 *
 * Created on 29 February 2024, 15:12
 */
...
void lcd_cmd(unsigned char cmd) // to SEND Command
 {
    LATC = 0x20 + ((cmd >> 4) & 0x0f);  // enable + MSB of instruction
    LATC = (cmd >> 4) & 0x0f;                  
// sent
    delay(1);
    LATC = 0x20 + (cmd & 0x0f);             
 ...
 {
    LATC = 0x30 + ((c >> 4) & 0x0f);    // enable + MSB of char
    LATC = 0x10 + ((c >> 4) & 0x0f);    // sent
    delay(1);
    LATC = 0x30 + (c & 0x0f);               
// enable + LSB of char
    LATC = 0x10 + (c & 0x0f);               
...
{ 
 ...
 lcd_cmd(0x0c);        // Turn display ON, no Cursor Displayed          (0000 1101)  
 lcd_cmd(0x06);        // Set Cursor to move right after each character (0000 0110)
 lcd_cmd(0x01);           // Clear the LCD Display                      (0000 0001)
 const unsigned char FirstRow_txt [16] = 
 { 'P', 'l', 'e', 'a', 's','e', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', 0x00 };
 const unsigned char SecondRow_txt [16] = 
 { 'E', 'N', 'T', 'E', 'R',' ','P', 'A', 'S', 'S', 'W', 'O', 'R', 'D', ':',  0x00 };
    while(1)
    {
     lcd_cmd(0x80);         // set to display at left of line 1 (0x80 + 0x00)
     display_message(FirstRow_txt);

     lcd_cmd(0xC0);         // set to display at left of line 2 (0x80 + 0x40)
     display_message(SecondRow_txt);
